...
#define TRIGGER_HIGH 650            // High limit for trigger, adjust to change false-trigger rejection
#define TRIGGER_LOW 575             // Low limit for trigger, adjust to change false-trigger rejection
#define TRIGGER_WAIT_TIME_MS 5     // Delay between triggers, lower to increase speed of triggering
...
inline void __attribute__((optimize("O3"))) update_output(const int level) {
    ...
    switch(logic_out) {
        case 0: {
            // If the level goes above TRIGGER_LOW, start sending output
            if(level >= TRIGGER_HIGH) {
                Serial.write("T\n");                // write value and flush
                Serial.flush();
                logic_out = 1;
                PORTB |= 0x20;                      // turn on builtin LED pin
                lastTriggerTime = millis();
            }
            break;
        }
        case 1: {
            // Keep sending output as long as the level is above TRIGGER_LOW
            if(level >= TRIGGER_LOW) {
                Serial.write("T\n");                // write value and flush
                Serial.flush();
                PORTB |= 0x20;                      // ensure LED remains on
                lastTriggerTime = millis();
            } 
            else if (millis() - lastTriggerTime >= TRIGGER_WAIT_TIME_MS) {
                // If the level drops below TRIGGER_LOW and the wait time has passed, stop sending output
                logic_out = 0;
                PORTB &= ~0x20;                     // turn off builtin LED pin (use bitwise AND to clear the bit)
            }
            break;
        }
    }
